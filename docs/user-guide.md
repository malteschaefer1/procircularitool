...
2. **Column mapping**: Map source headers (component, material, quantities) to the canonical schema using the mapping UI.
3. **Set parameters**: Choose at which level (product, component, material) each input is defined, then review or override only where needed; values cascade downward to reduce duplicate entry.
